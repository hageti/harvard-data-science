...
# Note: this process could take a couple of minutes, or even longer!
...
# Create a plot theme to apply to ggplot2 element text throughout this report
...
# Plot distribution of ratings in the edx dataset
  ...
  labs(x = "Rating", y = "Count (in millions)") + plot_theme
...
edx %>% group_by(movieId) %>%
  ...
  labs(x = "Average rating", y = "Number of movies") + plot_theme
...
edx %>% group_by(userId) %>%
  ...
  ggplot(aes(ave_rating)) +
  geom_histogram(bins=30, color = I("black")) +
  labs(x = "Average rating", y = "Number of users") + plot_theme
...
edx %>% 
  ...
  geom_histogram( bins=30, color = I("black")) +
  scale_x_log10() +
  labs(x = "Users", y = "Number of ratings") + plot_theme
...
edx %>% group_by(year) %>%
  ...
  geom_point() +
  geom_smooth() +
  labs(x = "Release Year", y = "Average Rating") + plot_theme
...
edx %>% group_by(year) %>%
  ...
  labs(x = "Release Year", y = "Number of Ratings (,000s)") + plot_theme
...
edx %>% group_by(review_date) %>%
  ...
  geom_point() +
  geom_smooth() +
  labs(x = "Date of Review", y = "Average Rating") + plot_theme
...
rmse_results <- data.frame(Method = "Project objective", RMSE = "0.86490", Difference = "-", stringsAsFactors = FALSE)
rmse_results
...
rmse_results %>% rbind(c("Simple average", round(simple_rmse,5), round(simple_rmse-rmse_objective,5)))
...
# Add result to the table
rmse_results %>% rbind(c("Movie effects", round(movie_rmse,5), round(movie_rmse-rmse_objective,5)))
...
# Add result to the table
rmse_results %>% rbind(c("Movie + User effects", round(user_rmse,5), round(user_rmse-rmse_objective,5)))
...
# Add result to the table
rmse_results %>% rbind(c("Movie + User + Genre effects", round(genre_rmse,5), round(genre_rmse-rmse_objective,5)))
...
# Add result to the table
rmse_results %>% rbind(c("Movie + User + Genre + Release Year effects", round(year_rmse,5), round(year_rmse-rmse_objective,5)))
...
# Add result to the table
rmse_results %>% rbind(c("Movie + User + Genre + Release Year + Review Date", round(review_rmse,5), round(review_rmse-rmse_objective,5)))
...
# Add result to the table
rmse_results %>% rbind(c("Regularised effects", round(regularised_rmse,5), round(regularised_rmse-rmse_objective,5)))

# Mutate validation dataset to reflect changes to edx (year, date of review) and run final hold-out test
...
# Add result to the table
rmse_results %>% rbind(c("Validation of Final Model", round(valid_rmse,5), round(valid_rmse-rmse_objective,5)))
